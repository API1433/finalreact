...
const GetBatch = () => {
    ...
    const getBatches = async () => {
        if (personal?.role === "instructor") {
            ...
            let filteredData = batches?.data.batches?.filter((data) => {
                if (new Date(data.EndDate) > newDate)
                    return { data }

            })
            setActiveBatch(filteredData)
            setFilterData(batches?.data.batches);
        }
        ...
        <>
            ...
            <div className="page-bottom">
                ...
                <Tabs defaultActiveKey="1" >
                    ...
                    <TabPane tab="Active Batches" key="2">
                        <Table
                            columns={columns}
                            dataSource={activeBatch}
                            bordered
                            className="usersTable"
                            scroll={{ x: 1400 }}
                            pagination={{
